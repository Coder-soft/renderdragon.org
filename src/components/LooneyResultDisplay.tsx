...
const SourceLink = ({ value, spotify = false, label }: { value: string; spotify?: boolean; label?: string }) => {
  ...
  return <a href={value} target="_blank" rel="noreferrer" title={value} className={`pixel-corners inline-flex max-w-full items-center gap-2 border px-3 py-2 text-sm font-semibold text-white ${spotify ? 'border-[#1ed760] bg-[#1ed760] hover:border-[#1fbd57] hover:bg-[#1fbd57]' : 'border-cow-purple/60 bg-cow-purple hover:border-cow-purple-dark hover:bg-cow-purple-dark'}`}><span className="pixel-corners relative flex h-4 w-4 shrink-0 items-center justify-center overflow-hidden bg-white/90 text-cow-purple"><IconExternalLink className="h-3 w-3" />{iconUrl && <img src={iconUrl} alt="" className="absolute inset-0 h-4 w-4 bg-white object-contain" onError={(event) => { event.currentTarget.style.display = 'none'; }} />}</span><span className="min-w-0 truncate">{spotify ? 'Open in Spotify' : label || hostLabel(value)}</span>{spotify ? <IconBrandSpotify className="h-4 w-4 shrink-0" /> : <IconExternalLink className="h-4 w-4 shrink-0" />}</a>;
};
...
const ResultValue = ({ value, linkLabel }: { value: LooneyValue; linkLabel?: string }) => {
  ...
  if (Array.isArray(value)) return <div className="min-w-0 space-y-2">{value.map((item, index) => <div key={index} className="pixel-corners min-w-0 break-words border border-border/60 bg-muted/30 p-3"><ResultValue value={item} /></div>)}</div>;
